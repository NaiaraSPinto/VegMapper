...
"""
A modified version of the Hyp3 download products script provided by the Alaska Satellite Facility.
This script downloads finished products one at a time from ASF and places them into an S3 bucket.

There are two possible options for downloading the finished products.
1. Downloading directly from Hyp3 using ASF username.
It takes as input the Alaska Satellite Facility username that has the products ready for download. 
It also takes a start date and end date for the range of dates which the products were processed. 
It also takes the name of the S3 bucket in which the put the files and the the name of the folder 
within the bucket where images should be placed. 

2. Downloading using a text file with the names of products ready to download. 
It takes as input a text file where each line contains the name of the product to download.
It also takes the name of the S3 bucket in which the put the files and the the name of the folder 
within the bucket where images should be placed. 

Example: 
1. python3 download-sentinel-images.py --download-by-username username start end sar-optical-images Sentinel1/
   python3 download-sentinel-images.py --download-by-username username 2020-09-24 2020-09-28 sar-optical-images Sentinel1/

2. python3 download-sentinel-images.py --download-by-file images_to_download.txt sar-optical-images Sentinel1/

TODO: 
1. Error handling for wrong inputs in arguments
2. Using --download-by-username asks for password twice (once to get url list and again to download)

"""

# TODO: Unzip file, get VV and VH, upload to folder and organize path and frame 
# TODO: get python script from Naiara that gets minimum overlapping area and calculates average of Sentinel images (gets rid of noise)
# Make sure all Sentinel images have the same dimension (over the same dimension)
# calculate min, max, mean, std. dev over a year's worth of sentinel images
# Change routines folder name to -> data prep 
# Collapse folder names and combine within single folder 
# Delete image_search_and_processing
# Give list of granule names via Python script and download it 
...
class bulk_downloader:
    def __init__(self, url_list):
        ...
        """ 
        The original script looks through the command line arguments for .metalink or .csv files 
        and uses those as the download links. Commented this out because I will be passing a 
        different arguments (see comment at top of file).
        """
    ...
    def download_files(self, file_name):
...
def get_url_list():
    # TODO: Error handling if file is not in correct format 
    if len(sys.argv) > 1:
        if sys.argv[1] == '--download-by-username':
            username = sys.argv[2]
            start = sys.argv[3]
            end = sys.argv[4]
            ...
            # list of dictionaries containing information of each product.
            product_list = api.get_products(start_date=start, end_date=end)

            # extract the download url of each product 
            url_list = [dic['url'] for dic in product_list]
        
        elif sys.argv[1] == '--download-by-file':
            with open(sys.argv[2], 'r') as f:
                url_list = [line.strip() for line in f.readlines()]

        else:
            print("Unkown argument: {}. Exiting...".format(sys.argv[1]))
            sys.exit()

        return url_list

    print("Wrong number of arguments. See comments at top of file. Exiting...")
    sys.exit()

def get_bucket_and_folder():
    if len(sys.argv) > 1:
        if sys.argv[1] == '--download-by-username':
            bucket, folder = sys.argv[5], sys.argv[6]
        
        elif sys.argv[1] == '--download-by-file':
            bucket, folder = sys.argv[3], sys.argv[4]
...
if __name__ == "__main__":
    ...
    s3_bucket, folder = get_bucket_and_folder()

    # initialize the downloader with the list of files to download
    downloader = bulk_downloader(url_list) 

    for url in downloader.files:
        file_name = url.split('/')[-1]
        file_path = os.path.join(folder, file_name)

        downloader.download_files(url)
        try:
            response = s3.upload_file(file_name, s3_bucket, file_path)
            bucket_path = os.path.join(s3_bucket, file_path)
            print("Successfully uploaded {} to {}.".format(file_name, bucket_path))
        except ClientError as e:
            print("S3 Upload Error:", e)

        os.remove(file_name) # remove file after downloading and uploading to S3
